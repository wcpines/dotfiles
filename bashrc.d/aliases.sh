#===========================#

#----------Aliases----------#
#===========================#

if [[ -n $(which bat) ]]; then
  alias cat="bat --theme auto:system"
fi

# alias rg="rg --hidden"
alias ....="cd ../.."
alias ..="cd .."
alias :q="exit"
alias :qa="exit"
alias ah="ls -gFash"
alias als="nvim $HOME/dotfiles/bashrc.d/aliases.sh"
alias blog="cd $HOME/Tresorit/Colby/blog/"
alias bp="nvim $HOME/dotfiles/bashrc.d"
alias brs="bin/rspec"
alias cc="claude -r"
alias dbh="nvim $HOME/dotfiles/bashrc.d/db.sh"
alias dbl="db_local"
alias dc="docker-compose"
alias desk="cd $HOME/Desktop"
alias dot="cd $HOME/dotfiles"
alias dt="desc_table"
alias eenv="nvim $HOME/.env"
alias esf="edit_searched_files"
alias fic="find_columns"
alias fo="fzf_file_opener"
alias focus="mix test --only focus"
alias focux="iex -S mix test --trace --only focus"
alias ft="find_tables"
alias funcs="nvim $HOME/dotfiles/bashrc.d/functions.sh"
alias gashd="git stash drop"
alias gcmn="git commit --amend --no-edit"
alias gcrb="git_current_branch"
alias gdi="git diff --ignore-all-space"
alias gdnd="git diff --name-only develop"
alias gdno="git diff --name-only"
alias gds="git diff --staged"
alias gdy="git diff --name-only develop | fzf | yank"
alias gforce="git push --force-with-lease origin HEAD"
alias gpld="git pull origin develop"
alias gplm="git pull origin master"
alias gpoh="git push origin HEAD"
alias grbd="git fetch origin develop:develop && git rebase develop"
alias grod="git rebase --onto develop HEAD~"
alias groh="git_reset_on_origin_hard"
alias hg="history_rg"
alias imes="iex -S mix ecto.seed"
alias imps="iex -S mix phx.server"
alias imt="iex -S mix test --trace"
alias ip="IPython3"
alias ism="iex -S mix"
alias ismn="iex --sname sched -S mix"
alias ismr='iex --sname debug --remsh sched@$(hostname -s)'
alias json="scratch json"
alias la="ls -laG"
alias ll="ls -lGh"
alias ls="ls -G"
alias mdg="mix deps.get"
alias mem="mix ecto.migrate"
alias mer="mix ecto.reset"
alias mes="mix ecto.seed"
alias mf="mix format"
alias minvim="nvim -u $HOME/dotfiles/vimrc.d/minimal.vim"
alias mk="open -a Marked.app"$1
alias mps="mix phx.server"
alias mt="mix test"
alias mtf="mix test --failed"
alias myip="ifconfig | grep -Eo 'inet (addr:)?([0-9]*\.){3}[0-9]*'"
alias o="open"
alias os="cd $HOME/Developer/onboarding-services/"
alias phx="mix phx.server"

alias ralph="bash $HOME/Developer/agent-config/loop/loop.sh"
alias rgd="rip_grep_delta"
alias rgf="rip_grep_files"
alias rgg="rip_grep_glob_search"
alias rgi="rip_grep_ignore_case"
alias rgl="rip_grep_glob_search_list"
alias rgm="rip_grep_migrations"
alias rm="rm -i"
alias sba="source $HOME/dotfiles/bashrc.d/aliases.sh"
alias sbp="source $HOME/.bash_profile"
alias srpd="set_rose_pine_dawn"
alias ssu="spotify share url"
alias stk="set_theme_kanagawa"
alias ta='tmux attach -t "$(tmux list-sessions -F "#S" | fzf)"'
alias tresor="cd $HOME/Tresorit/Colby"
alias tt="$HOME/dotfiles/ghostty/theme toggle"
alias tta="$HOME/dotfiles/ghostty/theme auto"
alias ttd="$HOME/dotfiles/ghostty/theme dark"
alias ttl="$HOME/dotfiles/ghostty/theme light"
alias wtl="wt list"
alias wtr="wt remove --no-delete-branch"
alias wts="wt switch"
alias xe="xargs nvim"
alias ya="yarn add"
alias yi="yarn install"
alias yrp="yarn run publish"
alias yrs="yarn run serve"
alias yrt="yarn run test"
alias ys="yarn start"
alias z="fuzzy_find_clipboard"

alias pibare="pi -nc --no-extensions --no-skills --no-prompt-templates --no-themes"
alias pic="pi -c"
alias pir="pi -r"

